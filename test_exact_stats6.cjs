  ...
  if (parsed.data && parsed.data.length > 0) {
    for (const row of parsed.data) {
      ...
      if (typeStr.includes('支出')) {
        ...
        if (category === "年度总收入" || category === "兼职收入" || category === "其他收入" || category === "生活费" || category === "红包/转账") {
...
console.log("csvTotalExpense (with more income categories):", csvTotalExpense);
console.log("csvTotalIncome (with more income categories):", csvTotalIncome);
