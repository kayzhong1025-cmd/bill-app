...
if (headerIndex !== -1) {
  ...
  if (parsed.data && parsed.data.length > 0) {
    for (const row of parsed.data) {
      ...
      if (category === '金融理财' || category === '资金流转') {
        financeRecords.push({ typeStr, category, amount, desc: row['商品说明'] || row['交易对方'] });
      }
    }
  }
  ...
  financeRecords.forEach(r => {
      if (r.typeStr.includes('支出')) totalFinanceExpense += r.amount;
  });
  console.log("Total Finance Expense:", totalFinanceExpense);
}
